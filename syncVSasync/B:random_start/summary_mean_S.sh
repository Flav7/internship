...
do
	...
	do
		cd 'sample'$j
		sum=`echo "scale=20;$sum+$(head -$i mean_S | tail -1)" | bc`
		cd ..
	done
	...
	echo -e -n $i"\t" >> summary_mean_S
	echo -e -n $mean"\t" >> summary_mean_S
	echo $sda >> summary_mean_S
